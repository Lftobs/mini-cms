...
export class ProjectService {
  ...
  async createProject(data: {
    ...
    if (!name || !repo_name || !github_repo_url || !orgs_id || !userId) {
      throw new ValidationError(
        "Name, repo_name, github_repo_url, orgs_id and userId are required",
      );
    }
  ...
  async checkProjectAccess(
    ...
    if (
      (accessType === "owner" || accessType === "both") &&
  ...
  async getProjectSettings(projectId: string, userId: string) {
    ...
    const dbValues = {
      ...
      public_directories: JSON.stringify(
        githubConfig.allowed_directories ?? [],
      ),
      ...
      allowed_extensions:
        settings?.allowed_extensions ?? JSON.stringify([".md", ".mdx", ".txt"]),
      require_approval: settings?.require_approval ?? true,
      auto_merge: settings?.auto_merge ?? false,
      collaborator_message:
        settings?.collaborator_message ??
        "Welcome! You can edit files in the allowed directories.",
    };
  ...
  async updateProjectSettings(
    ...
    const newConfig = {
      ...existingConfig,
      allowed_directories: Array.isArray(newDirectories)
        ? newDirectories
        : JSON.parse(newDirectories || "[]"),
    };
    ...
    if (typeof dbSettings.public_directories !== "string") {
      dbSettings.public_directories = JSON.stringify(
        dbSettings.public_directories,
      );
    }
  ...
  async inviteUser(
    ...
    const existingMember = await this.repository.findMemberByEmail(
      projectId,
      email,
    );
    if (existingMember) {
  ...
  async acceptInvite(token: string, userId: string) {
    ...
    const existingOrgMember = await this.repository.findOrgMember(
      project.orgId,
      userId,
    );
    ...
    const existingProjectMember = await this.repository.findMember(
      invite.projectId,
      userId,
    );
