...
const FileExplorer: React.FC<FileExplorerProps> = ({
  ...
  const loadFileTree = useCallback(async () => {
    ...
    try {
      ...
      const firstWithImagePath = config?.find(
        (c: DirectoryConfig) => c.base_image_path,
      );
            ...
            try {
              ...
              Object.entries(config.schema).forEach(
                ([key, schema]: [string, any]) => {
                  // Skip optional fields (required defaults to true if not specified)
                  ...
                  if (schema.type === "date") {
                    ...
                    let placeholder = "";
                    if (schema.min) placeholder = "x".repeat(schema.min);
