...
export default function ProjectSettingsModal({
	...
	const [allowedDirs, setAllowedDirs] = useState<any[]>([]);
	...
	const fetchSettings = async () => {
		...
		try {
			...
			if (res.ok) {
				...
				// Format for simple view or advanced view
				const simpleList = dirs.map((d: any) => typeof d === 'string' ? d : d.path).join("\n");
	...
	const handleOpen = () => {
		setIsOpen(true);
		// fetch starts immediately but doesn't block the modal UI
		fetchSettings();
	};
								...
								<div className="space-y-6">
									{loading ? (
										<div className="flex flex-col items-center justify-center py-12 space-y-3">
											...
											<div className="text-earth-300">Loading general settings...</div>
										</div>
									) : (
										<>
											<div>
												<div className="flex items-center justify-between mb-2">
													...
													<button
														onClick={() => {
															if (!isAdvanced) {
																...
																setRawAllowedText(JSON.stringify(allowedDirs, null, 2));
															} else {
																// Switch to simple: path list
																const simpleList = allowedDirs.map((d: any) => typeof d === 'string' ? d : d.path).join("\n");
																setRawAllowedText(simpleList);
															}
													...
													>
														{isAdvanced ? "Switch to Simple View" : "Advanced Configuration (JSON)"}
													</button>
												</div>
												<p className="text-xs text-earth-300 mb-4">
													{isAdvanced
														? "Edit the full configuration object including naming conventions and schemas for each directory."
														: "Enter one directory path per line (e.g., content/blog)."}
												</p>
												<textarea
													value={rawAllowedText}
													onChange={(e) => setRawAllowedText(e.target.value)}
													className="w-full h-64 px-3 py-2 border border-earth-200 rounded-md focus:outline-none focus:ring-2 focus:ring-earth-400 focus:border-transparent text-sm font-mono placeholder:text-earth-100"
												...
												{!isAdvanced && (
													<p className="mt-2 text-xs text-earth-300 italic">
														Tip: Switch to Advanced view to set up custom file schemas and naming rules.
													</p>
												)}
											</div>
